    ...
    if (usuario.autenticar()) {
        ...
        while (continuar) {
            ...
            case 7: {  // Eliminar Usuario
                vector<Usuario> usuarios = Usuario::leer_usuarios();
                if (usuarios.empty()) {
                    cout << "No hay usuarios registrados para eliminar.\n";
                    system("pause");
                    system("cls");
                    break;
                }
                ...
                for (const auto& usuario : usuarios) {
                    cout << "ID Usuario: " << usuario.id_usuario
                        << ", Nombre: " << usuario.nombre_usuario
                        << ", Rol: " << (usuario.rol == Usuario::PROPIETARIO ? "Propietario" : "Empleado") << endl;
                }
            ...
            case 1: {
                ...
                while (true) {
                    ...
                    // Verificar que el nombre tenga al menos un carácter alfabeticoy que no sea solo espacios
                    bool esValido = false;
                    for (char const& c : entrada) {
                        if (isalpha(c)) { // Verifica si hay al menos un carácter alfabético
                            esValido = true;
